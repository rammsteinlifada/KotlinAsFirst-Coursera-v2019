...
fun angleInRadian(deg: Int, min: Int, sec: Int): Double {
    var ans: Double
    ans = (deg.toDouble() + min.toDouble() / 60 + sec.toDouble() / 3600) * PI / 180
    return ans
}
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    var newPercent = 1 + percent.toDouble() / 100
    var ans: Double = initial.toDouble()
    for (i in 1..3){
        ans *= newPercent
    }
    return ans
...
fun numberRevert(number: Int): Int {
    ...
    var newNumber: Int = 0
    while (ourNumber > 0) {
        newNumber = newNumber * 10 + ourNumber % 10
        ourNumber = (ourNumber - ourNumber % 10) / 10
    }
    return newNumber
}
